...
def gen_dataset(index=3, n=20):
    file_path = os.path.join(Path(__file__).parents[1], "data", "raw", "grav", "gravitational_wave_signals.npy")
    base_target_path = os.path.join(Path(__file__).parents[1], "data", "parsed", "grav", f"grav-{index}")
    Path(base_target_path).mkdir(parents=True, exist_ok=True)
    for i in range(n):
        noisy_signals_plain, noisy_signals_anomalous, gw_signals = make_gravitational_waves(Path(file_path), n_signals=n)
