...
def extract_slice(file, start, end):
    file_path = os.path.join(Path(__file__).parents[1], "data", "raw", "hydro", file)
...
def parse_hydro(file, folder_prefix):
    ...
    np.save(os.path.join(file_parsed_folder, f"values-{folder_prefix}-z.npy"), values[2])
    save_preview_image(values[0], values[1], values[2], os.path.join(file_parsed_folder, f"{folder_prefix}.png"))


if __name__ == '__main__':
    parse_hydro("vibrationsdaten_Nov4-5_2022.parquet", "hydro-1")
