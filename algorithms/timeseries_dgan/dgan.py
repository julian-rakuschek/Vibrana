...
from algorithms.timeseries_dgan.errors import DataError, InternalError, ParameterError
from algorithms.timeseries_dgan.config import DfStyle, DGANConfig, OutputType
from algorithms.timeseries_dgan.structures import ProgressInfo
from algorithms.timeseries_dgan.torch_modules import Discriminator, Generator
from algorithms.timeseries_dgan.transformations import (
...
def _discrete_cols_to_int(
    ...
    if missing_discrete:
        logger.warning(
            f"The following discrete columns ({missing_discrete}) were not in the generated DataFrame, you may want to ensure this is intended!"
            # noqa
        )
...
class DGAN:
        ...
        if self.data_frame_converter is None:
            ...
            if df_style == DfStyle.WIDE:
                ...
                if example_id_column is None and attribute_columns:
                    raise ParameterError(
                        "Please provide an `example_id_column`, auto-splitting not available with only attribute columns."
                        # noqa
                    )
                if example_id_column is None and attribute_columns is None:
                    logging.warning(
                        f"The `example_id_column` was not provided, DGAN will autosplit dataset into sequences of size {self.config.max_sequence_len}!"
                        # noqa
                    )
                    if len(df) < self.config.max_sequence_len:
                        raise DataError(
                            f"Received {len(df)} rows in long data format, but DGAN requires max_sequence_len={self.config.max_sequence_len} rows to make a training example. Note training will require at least 2 examples."
                            # noqa
                        )
    ...
    def _train(
        ...
        self._set_mode(True)
        scaler = torch.amp.GradScaler('cuda', enabled=self.config.mixed_precision_training)
        ...
        for epoch in range(self.config.epochs):
            ...
            for batch_idx, real_batch in enumerate(loader):
                ...
                with torch.amp.autocast('cuda', enabled=self.config.mixed_precision_training):
