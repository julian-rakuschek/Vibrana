...
const SimpleChart = forwardRef(({chartId, data, width, height}: {
    chartId: string;
    data: number[];
    width?: number;
    height?: number
    ...
    const render = () => {
        d3.select(`#${id}`).datum({data: transformed, selected: {start: 100, end: 500}}).call(chart)
    };
    useEffect(() => {
        render()
    }, [data.length, chartId]);
...
export default function Home(): JSX.Element {
    ...
    const scatterRef = useRef<{ getSelectedPoint: () => number | undefined, setSelectedPoint: (value: number) => void }>(null);
    const [currentSelectedPoint, setCurrentSelectedPoint] = useState<number | undefined>(100);

    return (
        <DefaultPageWithBoundaries menuDarkMode>
            <p>{currentSelectedPoint}</p>
            {values.length > 0 && <SimpleChart data={values} chartId={"chart1"} height={200}/>}
            {projected.length > 0 && <SimpleScatter ref={scatterRef} onSelectedPointChange={(value) => setCurrentSelectedPoint(value)} data={projected} chartId={"scatter1"} height={600} width={"100%"}/>}
