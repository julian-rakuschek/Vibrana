...
export default function AnalyzePage(): JSX.Element {
    ...
    useEffect(() => {
        ...
        if (scatterRef && scatterRef.current && brushedRange) scatterRef.current.setRange(brushedRange)
    }, [brushedRange]);
    ...
    return (
        <DefaultPageWithBoundaries menuDarkMode>
            ...
            {projected.length > 0 &&
                <TimeSeriesPathView from_idx={brushedRange ? brushedRange[0] : 0} to_idx={brushedRange ? brushedRange[1] : values.length} ref={scatterRef} onSelectedPointChange={(value) => setCurrentSelectedPoint(value)}
                               data={brushedRange ? projected.slice(Math.floor(brushedRange[0] * projected.length), Math.floor(brushedRange[1] * projected.length)) : projected} chartId={"scatter1"} height={600} width={"100%"}/>}
