	...
	{:else}
		<div class="w-[350px] flex flex-col shadow-xl shrink-0 p-4 gap-4 h-full">
			...
			<div class="bg-indigo-100 rounded-xl p-4 gap-2 flex flex-col text-indigo-800">
				...
				<ThreadsControl {dataset} {subset} handleReset={() => fetchAndDrawAll()} handleSingleItem={addNewItem} bind:running />
				<div>
					<p class="text-center">
						Computation is
						<span class="font-semibold">{#if running}running{:else}paused{/if}</span>
					</p>
					<p class="text-center">{fingerprints.length} Fingerprints</p>
				</div>
			</div>
			<div class="bg-purple-100 rounded-xl p-4 flex flex-col gap-4">
				<p class="text-purple-800 font-semibold">Clustering Parameters</p>
			</div>
		</div>
